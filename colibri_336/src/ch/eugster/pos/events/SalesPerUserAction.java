 ...
 */
public class SalesPerUserAction extends ModeDependendAction implements IFailOverState
{
	 ...
	 * @param key
	 */
	public SalesPerUserAction(UserPanel context, Key key)
	{
		super(context, key);
	}
	 ...
	 * @param name
	 */
	public SalesPerUserAction(UserPanel context, Key key, String name)
	{
	 ...
	 * @param icon
	 */
	public SalesPerUserAction(UserPanel context, Key key, String name, Icon icon)
	{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			String value = NumberUtility.formatDefaultCurrency(Position.selectSales(User.getCurrentUser()), true, true);
	...
	public void modeChangePerformed(ModeChangeEvent e)
	{
		...
		if (this.getValue("enabled").equals(new Boolean(true))) //$NON-NLS-1$
		{
			...
			{
				// if
				// (this.context.getUser().status.equals(User.USER_STATE_ADMINISTRATOR))
				// {
				//					this.putValue("enabled", new Boolean(true)); //$NON-NLS-1$
				// }
				// else if
				// (this.context.getUser().status.equals(User.USER_STATE_MANAGER))
				// {
				//					this.putValue("enabled", new Boolean(true)); //$NON-NLS-1$
				// }
				// else
