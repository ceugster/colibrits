			...
			if (Database.temporary != null && Database.temporary.isActive() && Database.temporary.isConnected())
			{
				...
				if (receipts.length > 0)
				{
					...
					ProgressMonitor monitor = new ProgressMonitor(null,
									...
									Messages
													.getString("Database.Die_Belege_werden_aus_der_tempor_u00E4ren_Datenbank_in_die_Standarddatenbank_ntransferiert._Bitte_haben_Sie_etwas_Geduld..._18"), //$NON-NLS-1$
									0, receipts.length);
					monitor.setProgress(0);
					
					if (LogManager.getLogManager().getLogger("colibri") != null)
					{
						LogManager.getLogManager().getLogger("colibri").getLevel(); //$NON-NLS-1$
						LogManager.getLogManager().getLogger("colibri").setLevel(Level.INFO); //$NON-NLS-1$
						LogManager
										.getLogManager()
										.getLogger("colibri").info(Messages.getString("Database.Start_transfer__22") + receipts.length + Messages.getString("Database._Receipts_from_temporary_database_to_standard_database..._23")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					}
					...
					try
					{
						for (int i = 0; i < receipts.length; i++)
						{
							...
							if (LogManager.getLogManager().getLogger("colibri") != null)
							{
								LogManager.getLogManager().getLogger("colibri").info("Beleg wird eingelesen."); //$NON-NLS-1$ //$NON-NLS-2$
							}
											...
											|| receipts[i].status == Receipt.RECEIPT_STATE_REVERSED)
							{
								...
								if (LogManager.getLogManager().getLogger("colibri") != null)
								{
									LogManager
													.getLogManager()
													.getLogger("colibri").info(Messages.getString("Database.Galileodaten_werden_aktualisiert..._25")); //$NON-NLS-1$ //$NON-NLS-2$
								}
								Database.setCurrent(Database.standard);
								
								// 10163
								//								LogManager.getLogManager().getLogger("colibri").info("Beleg wird in Galileo verbucht."); //$NON-NLS-1$ //$NON-NLS-2$
								// bookGalileo(copy, booksNotFound);
								//								LogManager.getLogManager().getLogger("colibri").info("Beleg wird in Standarddatenbank kopiert."); //$NON-NLS-1$ //$NON-NLS-2$
								...
								if (saved[i].getErrorCode() == 0)
								{
									if (LogManager.getLogManager().getLogger("colibri") != null)
									{
										LogManager
														.getLogManager()
														.getLogger("colibri").info("Beleg wird in temporärer Datenbank gelöscht."); //$NON-NLS-1$ //$NON-NLS-2$
									}
							...
							MessageDialog
											.showInformation(
															...
															Messages.getString("Database.Beim_Transfer_von__28") + " " + receipts.length + " " + Messages.getString(Messages.getString("Database.Database._Belegen_aus_der_lokalen_in_die_Standarddatenbank_n_29_2")) + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
																			Messages
																							.getString("Database.wurden_Fehler_generiert_und_es_konnten_nicht_alle_Belege_transferiert_nwerden._30") + " " + //$NON-NLS-1$ //$NON-NLS-2$
																			Messages
																							.getString("Database.Die_Fehler_wurden_in_die_Protokolldatei_geschrieben._Nicht_transferierte_nBelege_31") + //$NON-NLS-1$
																			Messages
																							.getString("Database.werden_beim_n_u00E4chsten_Kassenstart_transferiert._32"), //$NON-NLS-1$
															0);
						}
						else
						{
								...
								MessageDialog
												.showInformation(
																...
																Messages.getString("Database.Es_sind__35") + inserted + Messages.getString("Database._Belege_erfolgreich_aus_der_lokalen_in_die_nStandarddatenbank_36") + //$NON-NLS-1$ //$NON-NLS-2$
																				Messages
																								.getString("Database._transferiert_worden._37"), //$NON-NLS-1$
																0);
							}
						}
					}
	...
	private static DBResult saveReceipt(Receipt receipt)
	{
		...
		if (result.getErrorCode() == 0)
		{
			...
			if (LogManager.getLogManager().getLogger("colibri") != null)
			{
				LogManager
								.getLogManager()
								.getLogger("colibri").info(Messages.getString("Database.Receipt_with_id______40") + receipt.getNumber() + Messages.getString("Database.___transferred._41")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			}
		}
		return result;
	}
	...
	private static DBResult removeReceipt(Receipt receipt)
	{
		...
		if (result.getErrorCode() == 0)
		{
			if (LogManager.getLogManager().getLogger("colibri") != null)
			{
				LogManager
								.getLogManager()
								.getLogger("colibri").info(Messages.getString("Database.Receipt_with_id______49") + receipt.getNumber() + Messages.getString("Database.___in_temporary_database_removed._50")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			}
		}
		else
		{
			if (LogManager.getLogManager().getLogger("colibri") != null)
			{
				LogManager
								.getLogManager()
								.getLogger("colibri").info(Messages.getString("Database.Problem_removing_Receipt_with_id______52") + receipt.getNumber() + Messages.getString("Database.___from_temporary_database..._53")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
