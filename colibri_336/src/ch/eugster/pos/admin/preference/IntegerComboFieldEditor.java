...
public class IntegerComboFieldEditor extends FieldEditor
{
	 ...
	 * @param values
	 */
	public IntegerComboFieldEditor(String name, String labelText, Composite parent, String[] list, int[] values)
	{
	...
	public void setItems(String[] items, int[] values)
	{
		this.values = values;
		int value = 0;
		if (this.combo.getSelectionIndex() == -1)
		{
			value = this.getPreferenceStore().getInt(this.getPreferenceName());
		}
		else
		{
	...
	private void selectionChanged(SelectionEvent e)
	{
		if (e.getSource().equals(this.combo))
		{
			...
			{
				this.doStore();
				this.fireValueChanged(this.getPreferenceName(), new Integer(oldValue), new Integer(this.newValue));
			}
		}
	}
