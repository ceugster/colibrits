 ...
 */
public class LongComboFieldEditor extends FieldEditor
{
	 ...
	 * @param values
	 */
	public LongComboFieldEditor(String name, String labelText, Composite parent, String[] list, long[] values)
	{
	...
	public void setItems(String[] items, long[] values)
	{
		this.values = values;
		long value = 0;
		if (this.combo.getSelectionIndex() == -1)
		{
			value = this.getPreferenceStore().getLong(this.getPreferenceName());
		}
		else
		{
	...
	protected void doFillIntoGrid(Composite parent, int numColumns)
	{
		...
		this.combo.addSelectionListener(new SelectionAdapter()
		{
			...
			{
				LongComboFieldEditor.this.selectionChanged(e);
			}
		});
	}
	...
	private void selectionChanged(SelectionEvent e)
	{
		if (e.getSource().equals(this.combo))
		{
			...
			long oldValue = this.newValue;
			this.newValue = this.values[this.combo.getSelectionIndex()];
			
			if (oldValue != this.newValue)
			{
				this.doStore();
				this.fireValueChanged(this.getPreferenceName(), new Long(oldValue), new Long(this.newValue));
			}
		}
	}
	...
	{
		long value = this.getPreferenceStore().getLong(this.getPreferenceName());
	...
	{
		long value = this.getPreferenceStore().getDefaultLong(this.getPreferenceName());
		for (int i = 0; i < this.values.length; i++)
		{
			if (this.values[i] == value)
			{
