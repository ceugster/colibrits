...
public abstract class StringButtonFieldEditor extends StringFieldEditor
{
	 ...
	 * <p>
	 * Subclasses must implement this method to provide a corresponding new
	 * string for the text field. If the returned value is <code>null</code>,
	...
	protected void doFillIntoGrid(Composite parent, int numColumns)
	{
		...
		gd.heightHint = this.convertVerticalDLUsToPixels(this.changeButton, IDialogConstants.BUTTON_HEIGHT);
		int widthHint = this.convertHorizontalDLUsToPixels(this.changeButton, IDialogConstants.BUTTON_WIDTH);
		gd.widthHint = Math.max(widthHint, this.changeButton.computeSize(SWT.DEFAULT, SWT.DEFAULT, true).x);
	...
	protected Button getChangeControl(Composite parent)
	{
		...
		{
			this.changeButton = new Button(parent, SWT.PUSH);
			if (this.changeButtonText == null) this.changeButtonText = JFaceResources.getString("openChange"); //$NON-NLS-1$
