...
public abstract class Table
{
	...
	private static Collection tryToSwitch(Query query)
	{
		...
		if (Database.getCurrent().equals(Database.getStandard()))
		{
			if (Database.isSwitchable() && Database.getTemporary().isConnected())
			{
				...
				if (Table.switchDatabase())
				{
					//											Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).severe(Messages.getString("Table.Datenbank_wurde_erfolgreich_gewechselt._48")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static void log(DBResult result, String table)
	{
		if (result.getErrorCode() != 0)
		{
			//			Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).info("Problem occured while updating table '" + table + "' in temporary database."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			//			Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).info(result.getErrorText()); //$NON-NLS-1$
			if (result.getExternalErrorCode() != null && result.getExternalErrorCode().length() > 0)
			{
				//				Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).info(result.getExternalErrorCode()); //$NON-NLS-1$
				//				Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).info(result.getExternalErrorText()); //$NON-NLS-1$
			}
		}
	}
