...
public class Settlement extends Table implements Comparable
{
	...
	private static DateFormat shortDateFormat = SimpleDateFormat.getDateInstance(DateFormat.SHORT);
	
	private static DateFormat mediumDateFormat = SimpleDateFormat.getDateInstance(DateFormat.SHORT);
	...
	public Settlement(Salespoint salespoint, Long settlement, Position position, int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, Position position, CurrentTax currentTax, int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, Payment payment, int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, int type, String text, int quantity, double amount,
					int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, Position position, int settlementType, int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, Receipt receipt, int receipts)
	{
	...
	public Settlement(Salespoint salespoint, Long settlement, int subtype, int cashtype, String code, double amount,
	...
	public Settlement(Salespoint salespoint, Long settlement, int subtype, int cashtype, String code, int quantity,
	...
	public Settlement(Salespoint salespoint, Long settlement, int subtype, int cashtype, String code, double value,
	...
	private Settlement(Position position, int receipts)
	{
		if (position.getProductGroup().type == ProductGroup.TYPE_INPUT)
		{
			...
			this.cashtype = position.getProductGroup().getForeignCurrency().getId().intValue();
		}
		...
		this.referenceClassName = position.getProductGroup().getClass().getName();
	...
	private Settlement(Position position, int type, int receipts)
	{
		...
		{
			this.shortText = position.getProductGroup().shortname + " " + position.getInvoiceNumber() + " "
							+ Settlement.shortDateFormat.format(position.getInvoiceDate());
			this.longText = position.getProductGroup().name + " " + position.getInvoiceNumber() + " "
	...
	private Settlement(int type, String text, int quantity, double amount, int receipts)
	{
		this.type = type;
	...
	private Settlement(int subtype, int cashtype, String code, double amount, String text, int receipts)
	{
	...
	private Settlement(int subtype, int cashtype, String code, int quantity, double amount, String text, int receipts)
	{
		this.type = Settlement.TYPE_CASH_CHECK;
		this.subtype = subtype;
		this.cashtype = cashtype;
	...
	private Settlement(int subtype, int cashtype, String code, double value, int quantity, double amount, String text,
					int receipts)
	{
		this.type = Settlement.TYPE_CASH_CHECK;
		this.subtype = subtype;
	...
	public static int countSettlements(Salespoint[] salespoints, Date from, Date to, boolean fullSelection)
	{
		...
		criteria.addNotEqualTo("type", new Integer(Settlement.TYPE_CASH_CHECK));
		
		if (!fullSelection)
		{
			Criteria simpleCriteria = new Criteria();
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_PAYED_INVOICES));
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_REVERSED));
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_TOOK_BACK));
			criteria.addAndCriteria(simpleCriteria);
		}
		
		if (salespoints != null) criteria.addAndCriteria(Salespoint.getSalespointCriteria(salespoints));
		
		QueryByCriteria query = new QueryByCriteria(Settlement.class, criteria);
		query.addOrderByAscending("salespointId");
		query.addOrderByAscending("type");
		query.addOrderByAscending("subtype");
		query.addOrderByAscending("cashtype");
		query.addOrderByDescending("value");
		query.addOrderByAscending("longText");
		query.addGroupBy("type");
		query.addGroupBy("subtype");
		query.addGroupBy("cashtype");
		query.addGroupBy("value");
		query.addGroupBy("longText");
		
		return Database.getCurrent().getBroker().getCount(query);
	}
	
	public static int countReceipts(Salespoint[] salespoints, Date from, Date to)
	{
		...
		ReportQueryByCriteria query = new ReportQueryByCriteria(Settlement.class, fields, criteria);
		query.setJdbcTypes(fieldTypes);
		
		query.addOrderByAscending("receipts");
		query.addGroupBy("receipts");
		...
		while (iter.hasNext())
		{
			...
			count += ((Integer) row[0]).intValue();
		}
		
		return count;
	}
	
	public static int countDiffRecords(Salespoint[] salespoints, Date from, Date to)
	{
		Criteria criteria = new Criteria();
		...
		criteria.addEqualTo("cashtype", new Integer(Settlement.CASH_CHECK_DIFFERENCE));
	...
	public static int countSettlements(Long settlement)
	{
		Criteria criteria = new Criteria();
	...
	public static Iterator selectDiffRecords(Salespoint[] salespoints, Date from, Date to)
	{
		String[] fields = new String[]
		{ "salespointId", "settlement", "code", "longText", "type", "subtype", "cashtype", "amount1" };
		
		int[] fieldTypes = new int[]
		{ Types.BIGINT, Types.BIGINT, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.INTEGER, Types.INTEGER,
						Types.DOUBLE };
	...
	public static Iterator selectSettlements(Salespoint[] salespoints, Date from, Date to, boolean fullSelection)
	{
		String[] fields = new String[]
		{ "salespointId", "type", "subtype", "cashtype", "longText", "code", "value", "receipts", "sum(quantity)",
						"sum(amount1)", "sum(amount2)", "settlement" };
		
		int[] fieldTypes = new int[]
		{ Types.BIGINT, Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.DOUBLE,
						Types.INTEGER, Types.INTEGER, Types.DOUBLE, Types.DOUBLE, Types.BIGINT };
		...
		{
			Criteria simpleCriteria = new Criteria();
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_INPUT_WITHDRAWAL));
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_PAYED_INVOICES));
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_REVERSED));
			simpleCriteria.addNotEqualTo("type", new Integer(Settlement.TYPE_TOOK_BACK));
			criteria.addAndCriteria(simpleCriteria);
	...
	public static Iterator selectSettlements(Long settlement)
	{
		String[] fields = new String[]
		{ "salespointId", "type", "subtype", "cashtype", "longText", "code", "value", "receipts", "quantity",
						"amount1", "amount2" };
	...
	public static Collection selectSettlementsForCouponPrinter(Long settlement)
	{
		Criteria criteria = new Criteria();
		...
		QueryByCriteria query = new QueryByCriteria(Settlement.class, criteria);
		query.addOrderBy("lineNumber", true);
		return Database.getCurrent().getBroker().getCollectionByQuery(query);
	}
	
	public static Long[] selectSettlementsNumbers(Salespoint[] salespoints, Date from, Date to)
	{
		String[] fields = new String[]
		{ "settlement" };
		...
		query.addOrderByDescending("settlement");
		query.setDistinct(true);
		
		Collection result = new ArrayList();
		Iterator iterator = Database.getCurrent().getBroker().getReportQueryIteratorByQuery(query);
